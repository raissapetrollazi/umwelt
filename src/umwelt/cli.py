...
def _parser() -> argparse.ArgumentParser:
    ...
    download = data_commands.add_parser(
        "download", help="Download pinned COMPASS files."
    )
    ...
    compare = commands.add_parser(
        "compare", help="Run replicated v0.1 temporal comparison."
    )
    compare.add_argument("--config", type=Path, required=True)
    compare.add_argument("--output", type=Path)

    individual = commands.add_parser(
        "individual", help="Run v0.1 population-variation comparison."
    )
    ...
    spatial = commands.add_parser(
        "spatial", help="Run the replicated v0.2 spatial experiment."
    )
...
def _data_download(arguments: argparse.Namespace, output: TextIO) -> int:
    results = download_compass(
        arguments.directory, include_all=arguments.all, repair=arguments.repair
    )
    _print_json(
        {
...
def _run(arguments: argparse.Namespace, output: TextIO) -> int:
    result = run_experiment(
        load_experiment_config(arguments.config), output_directory=arguments.output
    )
    _print_json(
        {
...
def _compare(arguments: argparse.Namespace, output: TextIO) -> int:
    result = run_temporal_lab(
        load_temporal_lab_config(arguments.config), output_directory=arguments.output
    )
    _print_json(
        {
            "experiment_id": result.experiment_id,
...
def _spatial(arguments: argparse.Namespace, output: TextIO) -> int:
    result = run_spatial_lab(
        load_spatial_lab_config(arguments.config), output_directory=arguments.output
    )
    _print_json(
        {
            "experiment_id": result.experiment_id,
